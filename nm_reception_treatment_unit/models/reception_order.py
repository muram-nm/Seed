...
class ReceptionOrder(models.Model):
    ...
    guests_count = fields.Integer(string='Guests', compute='_compute_guests_ids')
    ...
    @api.depends('reception_order_line.product_id.unit_id')
    def _compute_guests_ids(self):
        for order in self:
            order.guests_ids = self.env['unit.guest'].search(['|', ('reception_line_id', 'in', order.reception_order_line.ids), ('reception_order_id', '=', order.id)])
    ...
    @api.depends('reception_order_line.product_id', 'reception_order_line.unit_id')
    def _compute_unit_ids(self):
        for order in self:
            # order = order.reception_order_line.mapped('product_id.project_id')
            units = order.reception_order_line.mapped('unit_id')
 


            # order.project_count = len(projects)
            units = order.reception_order_line.mapped('product_id.unit_id')
            units |= order.reception_order_line.mapped('unit_id')
            # units |= order.unit_id
    ...
    def action_confirm(self):
        ...
        if self.reception_order_line:
            
            for line in self.reception_order_line:
                unit = self.env['unit.unit'].search([('project_id','=',line.product_id.project_id.id)],limit=1)
                if line.product_id.detailed_type == 'service' and line.product_id.service_tracking == 'task_global_project' and unit: 
                    line._create_unit_guest()    
        return result
