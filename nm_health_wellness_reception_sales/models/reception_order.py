...
class ReceptionOrder(models.Model):
	...
	def get_partner(self):
		for rec in self:
			if rec.customer_id:
				rec.partner_id = rec.customer_id.partner_id
	...
	def create(self, vals):
		...
		for order in res:
			if order.customer_id.partner_id not in order.message_partner_ids:
				order.message_subscribe([order.customer_id.partner_id.id])
...
class ReceptionOrderLine(models.Model):
	...
	order_partner_id = fields.Many2one(related='reception_order_id.customer_id.partner_id', store=True, string='Customer', index=True)
	...
	def _get_display_price(self, product):
		...
		product_context = dict(self.env.context, partner_id=self.reception_order_id.customer_id.id,
		...
		final_price, rule_id = self.reception_order_id.pricelist_id.with_context(
			product_context).get_product_price_rule(product or self.product_id, self.product_uom_qty or 1.0,
													self.reception_order_id.customer_id)
	...
	def product_id_change(self):
		if self.reception_order_id.pricelist_id and self.reception_order_id.customer_id:
			product = self.product_id.with_context(
				lang=self.reception_order_id.customer_id.lang,
				partner=self.reception_order_id.customer_id,
